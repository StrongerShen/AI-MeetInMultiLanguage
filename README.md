...
**系統能力與硬體資源：**
- 上傳限制設定為 **200 MB**。以 119 MB 錄音檔（約 1~2 小時）為例，解碼為原始 float32 音訊後佔用系統 RAM 約 460 MB，對於 48 GB 系統 RAM 完全無壓力。
- 嚴格的跨程序 **GPU 模型互斥卸載**，確保 Speaches (Breeze-ASR) 與 Ollama 絕不共存。Breeze 採用 30 秒切片處理，無論音檔長度，GPU 峰值皆恆定於 3.5 GB 左右，確保不發生 CUDA OOM。
...
### 需求與啟動方式
需求：Python 3.12、[uv](https://docs.astral.sh/uv/)、FFmpeg。若使用雲端轉錄才需設定 `OPENAI_API_KEY`；純本機推論（Breeze ASR + Ollama）無需金鑰。
...
uv run meet-eval speaches-transcribe ./var/eval/meeting/manifest.json ./var/results --host http://127.0.0.1:8001/v1 --profile breeze
uv run meet-eval ollama-benchmark eval/fixtures/meeting.zh-Hant-TW.txt ./var/ollama-benchmark --host http://127.0.0.1:11434 --model qwen3.5:9b
```
...
```
共有 111 項自動化測試，涵蓋跨行程 GPU 互斥、精確卸載、網路隔離防線、原始稿不可變、Range Request 串流等。評測資料、錄音與 `var/` 嚴禁提交至 Git。
...
- **嚴格連線防護 (Fail Closed)**：切換 GPU 模型時（包含 Ollama 探索、卸載、Speaches 卸載），只要發生 `ConnectError` 或逾時，系統視為狀態不明的潛在危險，一律拋出 `GpuTransitionError` 中止執行。
- **Python 測試網路防線 (Python Socket Isolation)**：測試環境預設拒絕當前 Python 程序中所有的 IPv4/IPv6 `socket.connect` 連線（包含 localhost），確保 Python 請求嚴格離線（此防線無法阻擋外部子程序）。
- **安全原子刪除 (TOCTOU 防護)**：刪除工作時，狀態檢查、音訊刪除與紀錄移除合併至同一跨程序鎖定交易。
...
為確保系統品質與安全，本專案歷經 Codex 嚴格的五輪程式碼審查。以下為過往審查與修正的時間軸摘要，所有曾經指出的高風險缺陷（如跨程序死鎖、連線異常洩漏、XSS 等）皆已於最新版本中徹底修復，請勿將審查歷史誤認為目前狀態：

- **第一輪 (啟動)**：由 `gpt-6-astra` 定義需求、混語策略與多模型架構。
- **第二輪 (實作)**：完成 P1 ~ P4 核心功能，包含長會議端到端批次壓測。
- **第三輪 (防護強化)**：針對跨程序 GPU 模型狀態、網路隔離與並行寫入提出改善要求。實作了 `conftest.py` 隔離防線與跨程序檔案鎖。
- **第四輪 (嚴格定義)**：針對連線失敗處理與測試網路隔離的定義提出質疑。全面移除 `ConnectError` 妥協，改採 Fail Closed 零容忍設計；並釐清測試網路防線僅針對 Python Socket。
- **第五輪 (正式簽核) - 2026-09-13**：Codex 確認已完整解決所有阻擋問題，未發現新的 GPU 互斥或資料一致性缺陷。111 項測試全數通過，正式簽核。
  - **下一步**：進入受控發布與實機驗收階段。
