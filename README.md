...
**P1-1 Breeze 管線穩定性基線（2026-09-14）**：82.6 分鐘真實會議錄音（119 MB MP3, SONY IC RECORDER）端到端 pipeline 驗收。管線穩定性基線通過；逐字稿與摘要品質待改善驗收。

| 項目 | 數值 |
| --- | --- |
| 音檔時長 | 4,956 秒（82.6 分鐘） |
| 音檔 SHA-256 | `48abf39878d380f68e2830867f04b99ee29365c438556136ee7a998ba1e9a51b` |
| 切段數 | 9 段（每段 600 秒） |
| 辨識段落 | 168 segments、19,514 字元 |
| ASR 耗時 | 約 328 秒（8 段各 39–53 秒 + 末段 3 秒） |
| 摘要耗時 | 約 96 秒（qwen3.5:9b、8 主題、4 待解問題、78 筆 evidence_ids） |
| 總耗時 | 498.9 秒（8.3 分鐘） |
| 完成後 GPU 顯存 | 213 MiB（與基線一致） |
| 品質警示 | 65/168 段（38.7%）有標記（suspected\_repetition\_hallucination: 58、high\_compression\_ratio: 57、high\_no\_speech\_probability: 8） |
| 匯出格式 | TXT (53 KB)、SRT (55 KB)、VTT (55 KB)、MD (63 KB)、JSON (143 KB) 全數完整 |
| raw\_asr 保護 | revision\_kind=raw\_asr、無後續修訂覆寫 |
| 失敗／重試 | 0 次 |

> **品質待驗收項目**：
> - 65/168 段（38.7%）有品質警示；原始逐字稿實際存在重複文字，需人工抽樣確認警示的精確率與漏報率。
> - 講者分離尚未啟用（speaker 全為 null）。
> - 摘要產出 0 筆 decisions / 0 筆 action\_items，屬於摘要品質待驗證項目，不得視為已通過。
> - CER/WER 需待人工標註集建立後正式量測。

### 評測機制說明

我們設計了兩套不同的評測機制，分別針對**底層字詞準確度**與**高層次邏輯摘要**進行客觀量測：

1. **語音辨識品質（CER 與 WER）**
   - 內建於 `meet-eval score` 工具中，使用**萊文斯坦距離（Levenshtein Distance）**計算「AI 產出」與「人工標準答案」之間的差異（新增、刪除、替換的次數）。
   - **CER（字錯率）**：適用於臺灣華語、臺語等以「單字字元」為單位的語言。公式為 `(錯誤總字元數) / 參考答案總字數`。
   - **WER（詞錯率）**：適用於純英語等以空白分隔「詞彙」為單位的語言。

2. **語者分離指標（DER，Diarization Error Rate）**
   - 用來評估「誰在什麼時候講話」的準確度。
   - **評分組成**：`(漏抓語音時間 + 幻覺多出時間 + 標錯講者時間) / 總會議時間`。
   - **實作方式**：可匯出 `.vtt` 等帶有時間軸與講者的字幕檔後，使用開源的 `pyannote.metrics` 工具進行外部時間軸與講者比對。

3. **摘要品質：證據有效率（Evidence Validity）**
   - 這是本系統的防護特色，用來**打擊 AI 幻覺（Hallucination）**，系統強制要求 AI 產出主題、決議與待辦摘要時，必須附上對應的 `evidence_ids`（如 `["seg-007"]`）作為事實溯源。
   - **自動檢驗（已實作）**：系統（`ollama_eval.py`）會將 JSON 摘要內的標籤與原始逐字稿交叉比對。若 AI 捏造了不存在的段落標籤，則程式會主動將其列為無效引用的 `unknown_evidence_ids`。

4. **摘要品質：決議與待辦召回率（Recall）**
   - 用來確保 AI **「沒有漏掉重要資訊」**。
   - **實作方式**：需先透過人工列出該場會議真正的核心決議與待辦事項作為 Reference，再檢視 AI 產出的 `action_items` 陣列是否有涵蓋這些項目，計算 `成功抓到的數量 / 真正的總數量`。

### 候選 ASR 與校訂技術研究

以下為 P1 階段候選 ASR 引擎與衍生校訂技術的研究紀錄。所有候選皆須以相同人工標註集實測後決定，不得預先宣稱任何候選已優於現有 Breeze ASR。

**1. P1 候選 ASR 引擎**

| 候選 | 定位 | 參考來源 |
| --- | --- | --- |
| Qwen3-ASR（1.7B） | 通用多語候選；支援中文、日文、英文等 30+ 語系 | [dseditor/QwenASRMiniTool](https://github.com/dseditor/QwenASRMiniTool) |
| TEA-ASR-1.1 | 乾淨臺灣華語／中英混用候選；原生繁體輸出（限乾淨人聲） | 同上 |
| Breeze-ASR-26 | 國臺混語候選（現有基線） | `paulpengtw/faster-whisper-Breeze-ASR-26` |

**2. 評測須比較項目**

- 中文／臺語 CER、英語 WER、日語字形正確率。
- 臺語關鍵詞與中英日混語正確率。
- 處理時間、GPU 顯存峰值、系統 RAM、失敗率。
- 模型輸出是否保留日語原生漢字——不得對原始辨識結果套用全域簡繁轉換。

**3. Forced Alignment（可選衍生資料）**

- 視為可選的衍生資料，用於字級時間軸、精細回聽與字幕校訂。
- 不得修改 `raw_asr`、原段落時間軸、`segment_id` 或原始逐字稿。
- 對齊失敗時保留既有段落級時間軸，不得自行偽造字級時間。

**4. 講者分離**

- 可研究獨立 ONNX diarization 元件。
- 必須獨立量測 DER 與跨片段講者一致性；不可因模型輸出而直接將講者身分視為事實。

**5. 明確列為目前不採用**

- **CrispASR／Vulkan 執行核心**：對方專案偏 Windows（WebView2、Win32 裝置偵測），本專案為 Ubuntu、Speaches、CUDA 架構，不能直接引入。
- **Cloudflare 對外通道、手機上傳、即時麥克風、卡拉 OK 模式**：非目前會後會議分析範圍，且對外服務須待 P2 的登入、TLS、授權與速率限制完成。
- **全域簡繁轉換**：不得影響日語原生漢字、臺語原文與 `raw_asr`。
